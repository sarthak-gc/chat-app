...
const Sidebar = ({ joinedGroups, chattedUsers }) => {
  ...
  const handleUserClick = (user) => {
    navigate(`users/${user._id}/detail`, {
      state: {
        user,
      },
    });
  ...
  return (
    <div className="w-96 bg-[#1a212c] h-screen relative flex-none  ">
        ...
        <span>
          {currentPage !== "settings" && (
            <>
              ...
              <input
                onBlur={() => setShowChattedUser(true)}
                ...
                return (
                  <>
                    <div
                      key={chattedUser._id}
                      onClick={() => {
                        handleUserClick(chattedUser);
                      }}
                    >
                      <div className="flex gap-4 p-2 h-16 text-[#A9B4C7] items-end ">
                        <img
                          src={`https://robohash.org/${chattedUser._id}?set=set5&size=100x100`}
                          alt={chattedUser.username[0].toUpperCase()}
                          className={`w-8 h-8 rounded-full`}
                        />
                        <div className="border-b border-[#dadada58] w-full pb-2">
                          <span>{chattedUser.username}</span>
                        </div>
                      </div>
                    </div>{" "}
                    <div
      ...
      <div>
        {users.length == 0 && !showChattedUser && <div>No user found</div>}
      </div>
      <div>
        {groups.length > 0 &&
          groups.map((group) => {
